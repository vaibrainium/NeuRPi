...
from NeuRPi.Protocols.RDK.hardware.behavior import Behavior
from NeuRPi.Protocols.RDK.hardware.hardware_manager import HardwareManager
    ...
    * **intertrial** - waiting period between two trials
    ...
    def __init__(self, stage_block=None, **kwargs):
        ...
        self.config = self.get_configuration(
            directory="Protocols/RDK/config", filename="dynamic_coherences"
        )
        ...
        self.behavior_manager = Behavior(
            hardware_manager=self.hardware_manager, stage_block=self.stage_block
        )
    ...
    def stimulus(self, *args, **kwargs):
        ...
        if self.task_pars.training_type.value < 0:  # 2:
            self.stimulus_time = (
                self.task_pars.training_type.active_passive.reaction_time_mu
        ...
        self.stage_block.wait()
        self.response, self.response_time = (
            self.behavior_manager.response,
            self.behavior_manager.response_time,
        )
        data = {
            "DC_timestamp": datetime.datetime.now().isoformat(),
            "trial_num": self.current_trial,
    ...
    def intertrial(self, *args, **kwargs):
        ...
        threading.Timer(self.ITI / 1000.0, self.stage_block.set).start()
...
class TrialManager:
    def __init__(self, TASK_PARAMS):
        ...
        self.rolling_bias = np.zeros(
            self.task_pars.bias.passive_correction.rolling_window
        )  # Initializing at no bias
    ...
    def generate_trials_schedule(self, *args, **kwargs):
        ...
        active_coherences = sorted(
            np.concatenate(
                [coherences[:coherence_level], -coherences[:coherence_level]]
            )
        )  # Signed coherence
    ...
    def next_trial(self, correction_trial):
        ...
        if correction_trial:
            ...
            self.stim_pars = {
                "coh": self.trials_schedule[self.schedule_counter]
                + np.random.choice([-1e-2, 1e-2])
    ...
    def graduation_check(self):
        ...
        # Deciding Coherence Values based on rolling performance (50 trials of each coherence)
        if App[
            "UI3"
        ].bidirectionalCoherences.isChecked():  # Bi-directional shift in coherence level
            if (
                (rolling_Perf["Accu_" + str(App["Coherences"][0])] > 0.7)
                and (rolling_Perf["Accu_" + str(App["Coherences"][-1])] > 0.7)
                and (rolling_Perf["Accu_" + str(App["Coherences"][1])] > 0.7)
                and (rolling_Perf["Accu_" + str(App["Coherences"][-2])] > 0.7)
            ):
                App[
                    "NextcohLevel"
                ] = 3  # If 100% and 70% coherence have accuracy above 70% increase coherence level to 3 i.e., introduce 36% coherence
                App["UI3"].meanpassiveRT.setValue(
                    20
                )  # Increasing mean RT for passive trials to 20 secs
                if (rolling_Perf["Accu_" + str(App["Coherences"][2])] > 0.7) and (
                    rolling_Perf["Accu_" + str(App["Coherences"][-3])] > 0.7
                ):
                    App[
                        "NextcohLevel"
                    ] = 4  # If 100%, 70% and 36% coherence have accuracy above 70% increase coherence level to 4 i.e., introduce 18% coherence
                    rolling_Perf[
                        "trial_counter_after_4th"
                    ] += 1  # Counting trials after 4th level coherence is introduced
                    App["UI3"].meanpassiveRT.setValue(
                        30
                    )  # Increasing mean RT for passive trials to 30 secs
                    if (
                        rolling_Perf["trial_counter_after_4th"] > 200
                    ):  # 200 trials after 4th level is introduced and above accuracy is mentained
                        App["NextcohLevel"] = 5  # lowest coherence is introduced
                    if (
                        rolling_Perf["trial_counter_after_4th"] > 400
                    ):  # 400 trials after 4th level is introduced and above accuracy is mentained
                        App["NextcohLevel"] = 6  # 0% coherence is introduced
                    if (
                        rolling_Perf["trial_counter_after_4th"] > 600
                    ):  # 400 trials after 4th level is introduced and above accuracy is mentained
                        pass  # to be written to remove 2nd level of coherence from list
                else:
                    rolling_Perf["trial_counter_after_4th"] = 0

        else:  # Uni-directional shift in coherence level
            if (App["UI1"].cohLevel.value() >= 3) or (
                (rolling_Perf["Accu_" + str(App["Coherences"][0])] > 0.7)
                and (rolling_Perf["Accu_" + str(App["Coherences"][-1])] > 0.7)
                and (rolling_Perf["Accu_" + str(App["Coherences"][1])] > 0.7)
                and (rolling_Perf["Accu_" + str(App["Coherences"][-2])] > 0.7)
                ...
                if (App["UI1"].cohLevel.value() >= 4) or (
                    (rolling_Perf["Accu_" + str(App["Coherences"][2])] > 0.7)
                    and (rolling_Perf["Accu_" + str(App["Coherences"][-3])] > 0.7)
                ):
                    App[
                        "NextcohLevel"
                    ] = 4  # If 100%, 70% and 36% coherence have accuracy above 70% increase coherence level to 4 i.e., introduce 18% coherence
