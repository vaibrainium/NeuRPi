...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--prefs",
        help="Location of .json prefs file (created during setup_autopilot.py)",
    )
    ...
    if prefs_orig.get("AUDIOSERVER") or "AUDIO" in prefs_orig.get("CONFIG"):
        if prefs_orig.get("AUDIOSERVER") == "pyo":
    ...
    * **PULLDOWNS** - Pin (board) numbers to pull down on boot.
    ...
    def __init__(self, splash=True, warn_defaults=True):
        ...
        if splash:
            try:
                welcome_msg = (
                    Path(__file__).resolve().parents[1] / "setup" / "welcome_msg.txt"
                )
        ...
        self.name = prefs_orig.get("NAME")
        super(Pilot, self).__init__(id=self.name)

        if prefs_orig.get("LINEAGE") == "CHILD":
            self.child = True
            self.parentid = prefs_orig.get("PARENTID")
        else:
            self.child = False
            self.parentid = "T"
        ...
        self.pulls = []
        if prefs_orig.get("PULLUPS"):
            for pin in prefs_orig.get("PULLUPS"):
                self.pulls.append(gpio.Digital_Out(int(pin), pull="U", polarity=0))
        if prefs_orig.get("PULLDOWNS"):
            for pin in prefs_orig.get("PULLDOWNS"):
    ...
    def l_start(self, value):
        ...
        try:
            ...
            prefs_orig.set("SUBJECT", self.subject)
    ...
    def l_cal_port(self, value):
        ...
        threading.Thread(
            target=self.calibrate_port, args=(port, n_clicks, open_dur, iti)
        ).start()
    ...
    def calibrate_port(self, port_name, n_clicks, open_dur, iti):
        ...
        """
        pin_num = prefs_orig.get("HARDWARE")["PORTS"][port_name]
    ...
    def l_cal_result(self, value):
        ...
        cal_fn = os.path.join(prefs_orig.get("BASEDIR"), "port_calibration.json")
    ...
    def l_stream_video(self, value):
        ...
        try:
            ...
            self.logger.exception(
                f"Expected camera id in form group.camera_id, got {camera}"
            )
            return

        if starting:
            if (
                cam_group in prefs_orig.get("HARDWARE")
                and cam_id in prefs_orig.get("HARDWARE")[cam_group]
            ):
                cam_prefs = prefs_orig.get("HARDWARE")[cam_group][cam_id]
                ...
                cam_obj.capture()
                self.logger.info(
                    f"Starting to stream video from {camera} to {stream_to}"
                )

            else:
                self.logger.exception(
                    f"No camera in group {cam_group} and id {cam_id} is configured in prefs"
                )
            ...
            if cam_group in self.hardware.keys() and cam_id in self.hardware[cam_group]:
                ...
                self.logger.exception(
                    f"No camera was capturing with group {cam_group} and id {cam_id}, have hardware {self.hardware}"
                )
    ...
    def calibration_curve(self, path=None, calibration=None):
        ...
        lut_fn = os.path.join(prefs_orig.get("BASEDIR"), "port_calibration_fit.json")
        ...
        if not calibration:
            ...
            if path:
                ...
                open_fn = os.path.join(
                    prefs_orig.get("BASEDIR"), "port_calibration.json"
                )
    ...
    def init_audio(self):
        ...
        * 'jack' = :class:`.jackclient.JackClient`
        ...
        elif prefs_orig.get("AUDIOSERVER") in ("jack", True):
    ...
    def blank_LEDs(self):
        ...
        """
        if "LEDS" not in prefs_orig.get("HARDWARE").keys():
            return

        for position, pins in prefs_orig.get("HARDWARE")["LEDS"].items():
    ...
    def open_file(self):
        ...
        """
        local_file = os.path.join(prefs_orig.get("DATADIR"), "local.h5")
        try:
        ...
        try:
            h5f.create_group(
                "/", self.subject, "Local Data for {}".format(self.subject)
            )
        ...
        if hasattr(self.task, "TrialData"):
            ...
            table = h5f.create_table(
                subject_group,
                datestring,
                table_descriptor.to_pytables_description(),
    ...
    def run_task(self, task_class, task_params):
        ...
        try:
            while True:
                ...
                data = next(
                    self.task.stages
                )()  # Double parens because next just gives us the function, we still have to call it
            ...
            self.logger.exception(
                f"got exception while running task, task stopping\n {e}"
            )
