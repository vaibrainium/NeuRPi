...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run an NeuRPi Terminal")
    parser.add_argument(
        "-f",
        "--prefs",
        help="Location of .json prefs file (created during setup_NeuRPi.py)",
    )
    args = parser.parse_args()

    if not args.prefs:
        prefs_file = "/usr/NeuRPi/prefs.json"

        if not os.path.exists(prefs_file):
            raise Exception("No Prefs file passed, and file not in default location")

        raise Warning(
            "No prefs file passed, loaded from default location. Should pass explicitly with -p"
        )
...
from NeuRPi.gui.menus.file import Protocol_Wizard
from NeuRPi.gui.menus.plugins import Plugins
from NeuRPi.gui.menus.swarm import Stream_Video
from NeuRPi.gui.menus.tests import Bandwidth_Test
from NeuRPi.gui.menus.tools import Calibrate_Water, Reassign, Weights
from NeuRPi.gui.plots.plot import Plot_Widget
from NeuRPi.gui.widgets.terminal import Control_Panel
from NeuRPi.networking import Net_Node, Terminal_Station
from NeuRPi.utils.invoker import get_invoker
from NeuRPi.utils.loggers import init_logger

# Try to import viz, but continue if that doesn't work
IMPORTED_VIZ = False
VIZ_ERROR = None
try:
    from NeuRPi import viz
...
class Terminal(QtWidgets.QMainWindow):
    ...
    **Listens used by the internal :class:`.Net_Node` **
    ...
    .. note::
        ...
        See :mod:`NeuRPi.prefs` for full list of prefs needed by terminal!

    .. note::

        The Terminal class is currently a subclass of :class:`PySide2.QtWidgets.QMainWindow` -- it will be refactored to
        inherit from :class:`~NeuRPi.agents.base.Agent` as the agent system is formalized.
    ...
    def __init__(self, warn_defaults=True):
        ...
        if warn_defaults:
            os.environ["NeuRPi_WARN_DEFAULTS"] = "1"
        ...
        self.settings = QtCore.QSettings(
            prefs_orig.get("TERMINAL_SETTINGS_FN"), QtCore.QSettings.NativeFormat
        )
        ...
        if len(self.pilots) == 0:
            box = pop_dialog(
                "No Pilots",
                "No pilots were found in the pilot_db, add one now?",
                buttons=("Yes", "No"),
            )
            ret = box.exec_()
            if ret == box.Yes:
    ...
    def initUI(self):
        ...
        * :class:`.plots.Plot_Widget`
        ...
        if "pytest" in sys.modules:
            ...
            terminal_winsize_behavior = prefs_orig.get("TERMINAL_WINSIZE_BEHAVIOR")
            custom_size = prefs_orig.get("TERMINAL_CUSTOM_SIZE")
        ...
        new_prot_act = QtWidgets.QAction(
            "New Pro&tocol", self, triggered=self.new_protocol
        )
        new_subject = QtWidgets.QAction(
            "New &Subject", self, triggered=self.new_subject
        )
        ...
        subject_weights_act = QtWidgets.QAction(
            "View Subject &Weights", self, triggered=self.subject_weights
        )
        update_protocol_act = QtWidgets.QAction(
            "Update Protocols", self, triggered=self.update_protocols
        )
        reassign_act = QtWidgets.QAction(
            "Batch Reassign Protocols", self, triggered=self.reassign_protocols
        )
        calibrate_act = QtWidgets.QAction(
            "Calibrate &Water Ports", self, triggered=self.calibrate_ports
        )
        ...
        stream_video = QtWidgets.QAction(
            "Stream Video", self, triggered=self.stream_video
        )
        self.swarm_menu.addAction(stream_video)

        # Plots menu
        self.plots_menu = self.menuBar().addMenu("&Plots")
        psychometric = QtWidgets.QAction(
            "Psychometric Curve", self, triggered=self.plot_psychometric
        )
        ...
        bandwidth_test_act = QtWidgets.QAction(
            "Test Bandwidth", self, triggered=self.test_bandwidth
        )
        ...
        if terminal_winsize_behavior == "maximum":
            ...
            if terminal_winsize_behavior != "moderate":
                self.logger.warning(
                    f'TERMINAL_WINSIZE_BEHAVIOR {terminal_winsize_behavior} is not implemented, defaulting to "moderate"'
                )
    ...
    def pilots(self) -> odict:
        ...
        if self._pilots is None:
            ...
            pilot_db_fn = Path(prefs_orig.get("PILOT_DB"))
    ...
    def protocols(self) -> list:
        ...
        protocols = os.listdir(prefs_orig.get("PROTOCOLDIR"))
    ...
    def new_protocol(self):
        ...
        if self.new_protocol_window.result() == 1:
            ...
            name, ok = QtWidgets.QInputDialog.getText(
                self, "Name Protocol", "Protocol Name:"
            )
            if ok and name != "":
                protocol_file = os.path.join(
                    prefs_orig.get("PROTOCOLDIR"), name + ".json"
                )
                ...
                protocol_file = os.path.join(
                    prefs_orig.get("PROTOCOLDIR"), placeholder_name + ".json"
                )
                with open(protocol_file, "w") as pfile_open:
                    json.dump(
                        save_steps,
    ...
    def new_subject(self):
        pop_dialog(
            "Not Implemented",
            details="Creating subjects from the File menu needs some cleaning up, in the meantime use the + button within a Pilot control panel",
            msg_type="warning",
        ).exec_()
        ...
        for subject in subjects:
            ...
            if any(protocol_bool):
                ...
                self.subjects[subject].assign_protocol(
                    os.path.join(prefs_orig.get("PROTOCOLDIR"), protocol),
    ...
    def reassign_protocols(self):
        ...
        if reassign_window.result() == 1:
            ...
            for subject, protocol in subject_protocols.items():
                ...
                if not protocol:
                    self.logger.info(
                        f"Protocol for {subject} set to blank, not setting"
                    )
                    continue

                self.subjects[subject].assign_protocol(protocol, step)
                self.logger.debug(
                    f"Assigned protocol {protocol}, step {step} to subject {subject}"
                )
    ...
    def calibrate_ports(self):
        ...
        if calibrate_window.result() == 1:
            for pilot, p_widget in calibrate_window.pilot_widgets.items():
                ...
                # send to pi
                self.node.send(to=pilot, key="CALIBRATE_RESULT", value=unnested_results)
    ...
    def plot_psychometric(self):
        ...
        if not IMPORTED_VIZ:
            _ = pop_dialog(
                "Vizualisation function couldn't be imported!", "error", VIZ_ERROR
            )
            return
        ...
        text, ok = QtGui.QInputDialog.getText(
            self, "save plot?", "what to call this thing"
        )
        if ok:
            chart.save(text)
