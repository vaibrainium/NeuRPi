...
class Enclosure(Data):
    ...
    box: Optional[typing.Union[str, int]] = Field(
        None, description="The number or name of the box this subject lives in, if any"
    )
    building: Optional[str] = Field(
        None, description="The name of the building that the subject is housed in"
    )
    room: Optional[Union[str, int]] = Field(
        None, description="The room number that the animal is housed in"
    )
...
class Genotype(Data):
    ...
    strain: Optional[str] = Field(
        None, description="The strain or background line of this subject, if any"
    )
    genes: Optional[List[Gene]] = Field(
        None, description="A list of any transgenes that this animal has"
    )
...
class Baselines(Data):
    ...
    minimum_pct: Optional[float] = Field(
        None,
    ...
    def minimum_mass(self) -> float:
        ...
        if self.mass is None or self.minimum_pct is None:
            raise ValueError(
                "Cant compute minimum mass without a baseline mass or minimum percent!"
            )
...
class Biography(Attributes):
    ...
    **Development Goals**
    ...
    id: str = Field(
        ...
        description="The indentifying string, name, subject_id, etc. for this subject. "
    ...
    start_date: Optional[datetime] = Field(
        ...
        description="The date that this subject file was created. Not that this is not necessarily the date "
    ...
    dob: Optional[datetime] = Field(
        None,
        description="The subject's date of birth. A datetime is allowed, but hours and minutes are typically not reliable. A time of midnight formally indicates "
        + "that the hour and minute is not precise.",
    )
    sex: SEX = Field(
        "U", description=f"Sex of the subject, one of {SEX}. See :data:`.SEX`"
    )
    description: Optional[str] = Field(
        None, description="Some lengthier description of the subject, idk go hogwild."
    )
    tags: Optional[dict] = Field(
        None,
        description="Any additional key/value tags that apply to this subject. Idiosyncratic metadata can be "
